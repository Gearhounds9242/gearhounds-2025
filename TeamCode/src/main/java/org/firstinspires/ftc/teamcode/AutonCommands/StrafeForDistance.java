...
public class StrafeForDistance extends Command {
    ...
    public StrafeForDistance(Hardware robot, double totalDistance, double riseDistance, double fallDistance, ElapsedTime timer, double timeOut, double powerLevel, double diff) {
        ...
        this.totalTicks = totalDistance * GearHoundsHardware.TICK_PER_INCH;
        this.riseTicks = riseDistance * GearHoundsHardware.TICK_PER_INCH;
        this.fallTicks = fallDistance * GearHoundsHardware.TICK_PER_INCH;
        if (powerLevel < 0) {
            powerDirection = -1.0;
        } else {
            powerDirection = 1.0;
    ...
    public void init() {
        if (getState() == STARTING && useInit) {
            ...
            FleftEndTicks=FleftStartTicks - (int)((inches*GearHoundsHardware.TICK_PER_INCH)*Math.signum(powerLevel));
            FrightEndTicks=FrightStartTicks - (int)((inches*GearHoundsHardware.TICK_PER_INCH)*Math.signum(powerLevel));

            BleftStartTicks = robot.leftBack.getCurrentPosition();
            BrightStartTicks = robot.rightBack.getCurrentPosition();
            BleftEndTicks=BleftStartTicks + (int)((inches*GearHoundsHardware.TICK_PER_INCH)*Math.signum(powerLevel));
            BrightEndTicks=BrightStartTicks + (int)((inches*GearHoundsHardware.TICK_PER_INCH)*Math.signum(powerLevel));
        }
        setState(RUNNING);
    }
    ...
    public void run() {
        if (getState() == RUNNING) {
            ...
            double power = GearHoundsHardware.MinPower;
            if (elapsedTime > timeOut) {
                ...
                if (d < riseTicks) {
                    // Rising power
                    power = ((powerLevel - GearHoundsHardware.MinPower)/riseTicks)*d + GearHoundsHardware.MinPower;
                } else if (d < totalTicks-fallTicks) {
                    // Constant power
                    power = powerLevel;
                } else {
                    // Falling power
                    power = ((GearHoundsHardware.MinPower-powerLevel)/(fallTicks))*(d - (totalTicks-fallTicks)) + powerLevel;
                }
